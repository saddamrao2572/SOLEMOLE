							...
							<div class="invoice-1">
								...
								<div class="invoice-1-add">
									<div class="invoice-1-add-left">
										...
										<h5>Contact</h5>
										<p>Email: <?= $details->contact; ?></p>
										
									</div>
									<div class="invoice-1-add-right">
										<ul>

											<li><span>Invoice Number</span><?= $details->invoice_no; ?></li>
...
<?php 
	   ...
	   $saledimei=\app\models\ShopProductSaledImei::find()->where(['nps_id'=>$details->id])->all();
	   foreach ($saledimei as $key ) {
	   	echo "<li>" . $key->saled_imei ."</li>";
	   }
										...
										<tbody>
											...
											<tr style="background-color: #253d52; ">
							                    ...
							                    if (!empty($details->paid) || !empty($details->discount)) {
							                    	...
							                    	// print_r($Remaining);
							                    	// exit();
	$sale = Yii::$app->db->createCommand("SELECT SUM(replace(price, ',', '')) FROM new_product_sale where id=$details->id;   ");
        $sales = $sale->queryScalar();
							...
							<div class="invoice-print no-print">
								...
								<a href="<?=url::to(['new-product-sale/index'])?>" class="waves-effect waves-light btn-large "><i class="fa fa-arrow-left"></i> Return</a>
								<a href="<?=url::to(['new-product-sale/update?id='.$details->id])?>" class="waves-effect waves-light btn-large "><i class="fa fa-pencil"></i> Edit</a>
